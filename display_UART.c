...
uint16_union modbus_rw_reg_rcv[RW_REG_LEN];			// read/write registers, modbus 1-15
uint16_union modbus_rw_reg_snd[RW_REG_LEN];			// read/write registers, modbus 1-15
uint16_union modbus_ro_reg_rcv[RO_REG_LEN];			// read only registers, modbus 201-220
uint16_union modbus_ero_reg_rcv[ERO_REG_LEN+1];		// extended read only registers, modbus 501-504 599
uint16_union modbus_ero_reg_snd[ERO_REG_LEN+1];		// extended read only registers, modbus 501-504 599
...
uint16 rx_invalid_packet_header()
{
	...
	if (dispRXBuffer[0]==0x03 &&
		(dispRXBuffer[1]==0x01 || dispRXBuffer[1]==0x03 || dispRXBuffer[1]==0x05 || dispRXBuffer[1]==0x06 )	)
...
//
//*****************************************************************************
bool display_uart_run()
{
	uint16 update_flags;				// temporary holding place for coil or register update flags
	...
	static uint8 send_cnt;				// bit position of the update flag being analyzed
...
uint16 UART_Return;
	...
	switch(state)
	{
		case 1:													// Read first piece of r/w registers
			error = display_reg_read(RW_REG_START, 8);
			if (!error)
			{
				for (i=0;i<8;i++)
				{
...
//		UART_Return = display_uart_update(REGISTER, WO_REG_START+DISPLAY_HARDWARE_REV, FALSE, 422, DISPLAY_HARDWARE_REV_F, 0);
		...
		case 2:													// Read second piece of r/w registers
			error = display_reg_read(FROST_ALARM_SP+RW_REG_START, 7);
			if (!error)
			{
				for (i=0;i<7;i++)
				{
					modbus_rw_reg_rcv[i+8].bytes.byte1 = dispRXBuffer[2*i+3];
					modbus_rw_reg_rcv[i+8].bytes.byte0 = dispRXBuffer[2*i+4];
				}
				clear_rx_buffer();
				state = 3;
			}
		...
		case 3:													// Read first piece of read only registers
		...
		case 4:													// Read second piece of read only registers
		...
		case 6:													// Write updated registers
		...
		case 7:													// Write updated coils
			...
			while (send_cnt<RW_COIL_LEN)
			{
				...
				if( 0x0001&update_flags )
				{
					update_flags = (uint16)modbus_rw_coil_snd[send_cnt/8];
		...
		case 8:													// Read production configuration registers (should read only)
		...
		case 9:													// Read production configuration registers (should read only)
			error = display_reg_read(ERO_REG_START+CURRENT_SETPOINT, 1);
			if (!error)
			{
				for (i=0;i<1;i++)
				{
					modbus_ero_reg_rcv[i+4].bytes.byte1 = dispRXBuffer[2*i+3];
					modbus_ero_reg_rcv[i+4].bytes.byte0 = dispRXBuffer[2*i+4];
				}
				clear_rx_buffer();
				state = 10;
			}
		...
		case 10:												// Write display firmware and hardware revision to be read by comm board
			tx_timeout = 0;										// restart multi-packet TX timeout timer
		...
		case 12:												// Write updated coils
			...
			while (send_cnt<ERW_COIL_LEN)
			{
				...
				if( 0x0001&update_flags )
				{
					update_flags = (uint16)modbus_erw_coil_snd[send_cnt/8];
					update_flags = update_flags & 0x0001<<shift;
					if(update_flags)
					{
						coil_value = COIL_TRUE;
...
//		case 13:													// Write updated registers
//			tx_timeout = 0;										// restart multi-packet TX timeout timer
//			while (send_cnt<WO_REG_DISP_REV_LEN)
//			{
//				update_flags = write_rev_reg_flags;				// store update registers flags in temporary location
//				update_flags >>= send_cnt;						// shift temporary flags
//				if( 0x0001 & update_flags )
//				{
//					error = display_reg_single_write(RW_REG_START+send_cnt, modbus_wo_reg_snd_disp_rev[send_cnt].uvalue );
//					if (!error)
//					{
//						send_cnt++;								// increment if tx is successful
//					}
...
//				if (tx_timeout>=TX_MAX_WAIT)					// time out on transmit if not complete in x seconds.
//				{
//					send_cnt = WO_REG_DISP_REV_LEN;
//				}
//			}
//			if (send_cnt>=WO_REG_DISP_REV_LEN && tx_timeout<TX_MAX_WAIT)
//			{
//				clear_rx_buffer();
//				write_rev_reg_flags = 0;						// clear all flags if multi-packet TX is successful
//				send_cnt = 0;
		...
		uint8	coil_flags)				// coil flags to update
{
	if ( type==COIL )
	{
		...
		{
			if ( address<=RW_COIL_END )
			{
				address -= RW_COIL_START;
				if ( coil_state )
				{
					modbus_rw_coil_snd[address/8] |= coil_flags;
				}
				else
				{
					modbus_rw_coil_snd[address/8] &= ~coil_flags;
			...
			else if ( address>=ERW_COIL_START && address<=ERW_COIL_END )
			{
				...
				if ( coil_state )
				{
					modbus_erw_coil_snd[address/8] |= coil_flags;
				}
				else
				{
					modbus_erw_coil_snd[address/8] &= ~coil_flags;
		...
		uint16 length)					// Length of the data to read
{
	...
	if ( (address>=RW_REG_START					&& address<=RW_REG_END)						 ||
		 ...
		 (address>=(ERO_REG_START+HPC_ACTIVATE) && address<=(ERO_REG_START+MALF_DEACTIVATE)) ||
		  address==(ERO_REG_START+CURRENT_SETPOINT) )
	{
		adr.uvalue = address;
